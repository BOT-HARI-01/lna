...
const TagSelector = () => {
  ...
  useEffect(() => {
    const handleUserLog = () => {
      ...
      if (storedUser && storedUser.preferences && storedUser.headings) {
        setSelectedTags(storedUser.preferences);
        setSelectedHeadings(storedUser.headings);
  ...
  const onTagSelection = async (tag) => {
    ...
    try {
      const response = await fetch('/api/update-preferences', {
        ...
        body: JSON.stringify({headings : selectedHeadings, tags: updatedTags, username: userName })
      });
      const data = await response.json();
      const storedUser = JSON.parse(localStorage.getItem('user'));
      if (storedUser) {
        storedUser.preferences = updatedTags;
        storedUser.headings = selectedHeadings;
